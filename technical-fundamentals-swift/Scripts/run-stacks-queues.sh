...
echo -e "${BLUE}${BOLD}${ROCKET} Running Swift Stacks and Queues Problems Tests${NC}"
echo "================================================="

# Change to the Swift project directory
cd "$(dirname "$0")/.."

# Array of stacks and queues problem test files
stacks_queues_tests=(
    "ThreeStacksTests"
    "StackMinTests"
    "StackOfPlatesTests"
...
echo -e "\n${BOLD}Stacks and Queues Problems Test Results:${NC}"
echo "================================="

# Run each test and capture results
for test in "${stacks_queues_tests[@]}"; do
...
if [ ${#failed_tests[@]} -eq 0 ]; then
    echo -e "${GREEN}${BOLD}${CELEBRATION} All stacks and queues tests passed! Great job! ${CELEBRATION}${NC}"
    
    # Generate a simple hash for "password" like the TypeScript version
    hash_input=$(echo "${stacks_queues_tests[@]}" | tr -d ' ')
...
echo "Example: swift test --filter StackQueueTests"