...
echo -e "${BLUE}${BOLD}${ROCKET} Running Swift LinkedList Problems Tests${NC}"
echo "================================================="

# Change to the Swift project directory
cd "$(dirname "$0")/.."

# Array of linkedlist problem test files
linkedlist_tests=(
    "LinkedListNodeTests"
    "RemoveDupsTests"
    "KthToLastTests"
...
echo -e "\n${BOLD}LinkedList Problems Test Results:${NC}"
echo "================================="

# Run each test and capture results
for test in "${linkedlist_tests[@]}"; do
...
if [ ${#failed_tests[@]} -eq 0 ]; then
    echo -e "${GREEN}${BOLD}${CELEBRATION} All linkedlist tests passed! Great job! ${CELEBRATION}${NC}"
    
    # Generate a simple hash for "password" like the TypeScript version
    hash_input=$(echo "${linkedlist_tests[@]}" | tr -d ' ')
...
echo "Example: swift test --filter LinkedListNodeTests"